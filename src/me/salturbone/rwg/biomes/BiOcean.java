...
public class BiOcean {
    ...
    public void draw(int x, int y, int z, int chunk_x, int chunk_z) {
        ...
        if (c_noise < -0.1 && c_noise >= -0.15) {
            ...
            h = (int) (y + (-1 * Math.abs(draw_noise.noise(chunk_x * 16 + x, chunk_z * 16 + z) * 10)));
            chunk.setBlock(x, h - 1, z, Material.SAND);
            for (int i = h; i < ocean_limit; i++) {
                chunk.setBlock(x, i, z, Material.AIR);
            }
        }
        if (c_noise < -0.15) {
            biome.setBiome(x,z, Biome.DEEP_OCEAN);
            h = (int) (y + (-1 * Math.abs(draw_noise.noise(chunk_x * 16 + x, chunk_z * 16 + z) * 30)));
            chunk.setBlock(x, h - 1, z, Material.SAND);
            for (int i = h; i < ocean_limit; i++) {
                chunk.setBlock(x, i, z, Material.AIR);
            }
        ...
        if (h < ocean_limit) {
            for (int i = y + 1; y < ocean_limit; i++) {
                chunk.setBlock(x, h, z, Material.WATER);
            }
        }
